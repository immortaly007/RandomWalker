...
public class RoadMap implements ReadOnlyRoadMap {
	...
	public void addNode(OsmNode node) {
		addNode(node.getId(), new MapNode(new LatLonCoordinate(node.getLatitude(), node.getLongitude())));
	}
	
	public void addNode(Long id, MapNode node) {
		nodes.put(id, node);
	}
	...
	public void addWay(List<Long> way) {
		...
		MapNode lastNode = nodes.get(lastNodeId);
	...
	public void clean() {
		...
		for(Iterator<Map.Entry<Long, MapNode>> nodeIterator = nodes.entrySet().iterator(); nodeIterator.hasNext();) {
			Map.Entry<Long, MapNode> nodeEntry = nodeIterator.next();
	...
	public List<Long> findNodesNear(LatLonCoordinate pos, double maxDistance) {
		...
		for (Map.Entry<Long, MapNode> nodeEntry: nodes.entrySet()) {
	...
	public static class MapNode implements ReadOnlyRoadMapNode {
		
		private final LatLonCoordinate position;
		Map<Long, Double> connections;
		
		public MapNode(LatLonCoordinate position) {
